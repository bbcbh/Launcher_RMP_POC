...
public class Util_Analyse_RMP {
	...
	public static void extracted_morbidity_from_InfectHist(File[] sce_dir, File output_dir,
		...
		loop_sce_dir: for (File resultSetDir : sce_dir) {
			File[] singleResultSets = resultSetDir.listFiles(new FileFilter() {
				@Override
				public boolean accept(File pathname) {
					return pathname.isDirectory() && !"PBS".equals(pathname.getName());
										...
										if (morbidity_prob_map.containsKey(inf_id)) {
											...
											for (int inf_hist_index = 2; (inf_hist_index
												...
												if (treatment_type != Runnable_MetaPopulation_Transmission_RMP_MultiInfection.INFECTION_HIST_OVERTREATMENT) {
													...
													pt_sample_time = Math.min(pt_sample_time, sample_time.length - 1);
													...
													while (pt_sample_time >= 1
														...
														Number[] morbidity_event = new Number[] {
																prob_morbidity_per_day, morbidity_dur_sample_time_start,
				...
				for (String morbid_key : area_under_curve_all.keySet()) {
					for (Entry<Integer, HashMap<String, HashMap<Integer, ArrayList<Number[]>>>> area_under_curve : area_under_curve_all
						...
						if (area_under_curve.getValue().size() > 0) {
							String priKey = String.format("%s_%s", morbid_key, area_under_curve.getKey());
							PrintWriter pWri_morbidity_stat = pri_map.get(priKey);
							if (pWri_morbidity_stat == null) {
								pWri_morbidity_stat = new PrintWriter(
								...
								pWri_morbidity_stat.println(header.toString());
								pri_map.put(priKey, pWri_morbidity_stat);
							...
							for (String sim_key : sim_keys) {
								area_under_curve_by_morbidity_inf_sim = area_under_curve.getValue().remove(sim_key);
								...
								if (time_range_index[0].equals(0)) {
									time_range_index = Arrays.copyOfRange(time_range_index, 1, time_range_index.length);

								}
								pWri_morbidity_stat.printf("%s", sim_key);
								for (Integer tR_i : time_range_index) {
									...
									pWri_morbidity_stat.printf(",%d,%s",
											area_under_curve_by_morbidity_inf_sim.get(tR_i).size(), pSum);
								}
								pWri_morbidity_stat.println();
							}											
							pWri_morbidity_stat.flush();
						}
					}
				}

				resultSet_count++;				
//				if (resultSet_count > 5) {
//					break loop_sce_dir;
//				}
...
//
//				if (area_under_curve.getValue().size() > 0) {
//					PrintWriter pWri_morbidity_stat = new PrintWriter(
//							new File(output_dir, String.format(morbid_key, area_under_curve.getKey())));
//					
//					
//					
...
//						
//						if(time_range_index[0].equals(0)) {
//							time_range_index = Arrays.copyOfRange(time_range_index,									
//									1, time_range_index.length);
//									
//						}
//
//						pWri_morbidity_stat.printf("%s", sim_key);
...
//										morbidity_stat[2].intValue() - morbidity_stat[1].intValue());
//							}
//							pWri_morbidity_stat.printf(",%d,%s", area_under_curve_by_morbidity_inf_sim.get(tR_i).size(),
//									pSum);
//						}
//
//						pWri_morbidity_stat.println();
